...
class StreamsEventBuilder {
    ...
    private fun buildPayload(payload: Any?): Any? {
        if (payload == null) {
            return null
        }
        return when (payload) {
            ...
            is Path -> {
                ...
                val rels = payload.relationships().map { buildPayload(it) }
                val nodes = payload.nodes().map { buildPayload(it) }
                mapOf("length" to length, "rels" to rels, "nodes" to nodes)
            }
            is Map<*, *> -> {
                payload.mapValues { buildPayload(it.value) }
            }
            is List<*> -> {
                payload.map { buildPayload(it) }
            }
            else -> {
                payload
            }
    ...
    fun build(): StreamsEvent {
        return StreamsEvent(buildPayload(payload)!!)
    }
